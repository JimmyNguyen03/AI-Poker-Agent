...
# Allow `python offline_learning/self_play.py` from repo root.
...
class LoggingMCTSPlayer(MCTSPlayer):
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        action_or_pair = super().declare_action(valid_actions, hole_card, round_state)
        chosen_action = action_or_pair[0] if isinstance(action_or_pair, tuple) else action_or_pair
        state = build_state(
            hero_uuid=self.uuid,
